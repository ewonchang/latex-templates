...
\usepackage{setspace} %  \singlespacing, \doublespacing, \onehalfspacing and \setstretch{baselinestretch}
\newcommand{\comment}[2]{#2} % for adding long comments. \comment{This is a long comment}
\usepackage{blindtext} % add ramdom latin text
...
\newpage % Declaration of Originality (Page III, shown)
...
\chapter*{Abstract} % Abstract (page IV, shown)
...
\tableofcontents
\listoftables
\addcontentsline{toc}{chapter}{List of Tables} % add an entry to ToC
\listoffigures
\addcontentsline{toc}{chapter}{List of Figures} % add an entry to ToC


% (Optional) LIST OF APPENDICES

% (Optional) LIST OF abbreviations/SYMBOLS
...
\doublespacing % refer to \usepackage{setspace}
...
% Reference/Bibliography goes here

\bibliographystyle{ieeetr} % IEEEtran.bst has url 
\bibliography{zotero} % use .bib file auto-generated by Zotero-better-bibtex
